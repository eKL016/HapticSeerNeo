...
namespace XBoxInputWrapper
{
    partial class Program
    {
        ...
        {
            if (args.Length != 1) throw new ArgumentException("No outlet channel specified");
            using (publisher = new Publisher(args[0]))
            {
        ...
        {
            publisher.PublishToOutlet($"{SourceEvent.ToString()}|{EventInfo}");
#if DEBUG
