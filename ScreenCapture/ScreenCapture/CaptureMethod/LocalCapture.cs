...
namespace ScreenCapture
{
    public class LocalCapture : CaptureMethod
    {
        ...
        public LocalCapture(int numOutput = 0, int numAdapter = 0)
        {
            ...
            // Create a temp array
            tempArray = new Int32[Width * Height];
            producer = new Producer<int>("VIDEO", width*height, 10);
            ...
            while (!cancellation.IsCancellationRequested)
            {
                try
                {
                    ...
                    */
                    ...
                    GCHandle handle = GCHandle.Alloc(tempArray, GCHandleType.Pinned);
                    using (Mat headerOnly = new Mat(new System.Drawing.Size(Width, Height), DepthType.Cv8U, 4, handle.AddrOfPinnedObject(), Width * 4))
                    {
                        Copy_32Argb_ImageTo_32Argb_Mat(in mapSource, in headerOnly);
